import json
import os
import glob
import argparse
import logging
import matplotlib.pyplot as plt
import matplotlib.gridspec as gridspec
import numpy as np
import pandas as pd
from datetime import datetime
from matplotlib.patches import Patch

def get_min_profit_threshold(exchange_name):
    """Загружает MIN_PROFIT_THRESHOLD из соответствующего файла конфигурации."""
    try:
        if exchange_name == 'huobi':
            from config import MIN_PROFIT_THRESHOLD
            return MIN_PROFIT_THRESHOLD
        elif exchange_name == 'binance':
            from config_binance import MIN_PROFIT_THRESHOLD
            return MIN_PROFIT_THRESHOLD
    except (ImportError, ModuleNotFoundError):
        print(f"Warning: Could not import config for {exchange_name}. Using default threshold 0.1.")
    return 0.1 # Значение по умолчанию

def generate_report(df: pd.DataFrame, filename: str, exchange_name: str, new_tickers=None):
    if df.empty:
        logging.warning("DataFrame is empty. Generating an empty report.")
        fig, ax = plt.subplots(figsize=(12, 6))
        fig.suptitle(f'Triangular Arbitrage Analysis - {exchange_name}', fontsize=16, weight='bold')
        ax.text(0.5, 0.5, 'No market divergence data was collected.',
                horizontalalignment='center', verticalalignment='center', fontsize=18, color='grey')
        ax.set_axis_off()
        plt.savefig(filename, dpi=150, bbox_inches='tight')
        plt.close()
        return

    unique_paths = df['path'].unique()
    num_paths = len(unique_paths)
    if num_paths == 0: return

    min_profit_threshold = get_min_profit_threshold(exchange_name.lower())

    # --- Начало генерации отчета ---
    ncols = 3
    nrows_paths = (num_paths + ncols - 1) // ncols
    
    # Определяем, есть ли данные о задержке, и корректируем сетку
    has_latency_data = 'api_latency' in df.columns and not df['api_latency'].isnull().all()
    total_rows_grid = 2 if has_latency_data else 1
    height_ratios = [1, 3] if has_latency_data else [1]

    fig = plt.figure(figsize=(25, 4 * total_rows_grid + 6 * nrows_paths))
    fig.suptitle(f'Triangular Arbitrage Analysis - {exchange_name.capitalize()}', fontsize=22, y=0.98)
    main_grid = gridspec.GridSpec(total_rows_grid, 1, hspace=0.4, height_ratios=height_ratios)

    # --- Секция анализа задержки API ---
    if has_latency_data:
        ax_latency_hist = fig.add_subplot(main_grid[0])
        latency_data = df['api_latency'].dropna()
        mean_latency = latency_data.mean()
        median_latency = latency_data.median()
        std_latency = latency_data.std()

        ax_latency_hist.hist(latency_data, bins=50, color='skyblue', alpha=0.8, label='Latency Distribution')
        ax_latency_hist.axvline(mean_latency, color='red', linestyle='--', linewidth=2, label=f'Mean: {mean_latency:.2f} ms')
        ax_latency_hist.axvline(median_latency, color='green', linestyle='--', linewidth=2, label=f'Median: {median_latency:.2f} ms')
        
        ax_latency_hist.set_title('API Latency Distribution', fontsize=16, weight='bold')
        ax_latency_hist.set_xlabel('Latency (ms)', fontsize=12)
        ax_latency_hist.set_ylabel('Frequency', fontsize=12)
        ax_latency_hist.legend()
        ax_latency_hist.grid(True, linestyle='--', alpha=0.6)
        
        stats_text_latency = (
            f'Mean: {mean_latency:.2f} ms\n'
            f'Median: {median_latency:.2f} ms\n'
            f'Std Dev: {std_latency:.2f} ms\n'
            f'Min: {latency_data.min():.2f} ms\n'
            f'Max: {latency_data.max():.2f} ms'
        )
        ax_latency_hist.text(0.95, 0.95, stats_text_latency, transform=ax_latency_hist.transAxes, fontsize=12,
                             verticalalignment='top', horizontalalignment='right',
                             bbox=dict(boxstyle='round,pad=0.5', fc='wheat', alpha=0.5))

    # --- Секция анализа арбитражных путей ---
    paths_grid_spec = main_grid[1] if has_latency_data else main_grid[0]
    outer_grid = gridspec.GridSpecFromSubplotSpec(nrows_paths, ncols, subplot_spec=paths_grid_spec, wspace=0.2, hspace=0.5)

    for i, path_name in enumerate(unique_paths):
        path_df = df[df['path'] == path_name]
        profits = path_df['profit_percentage']
        opportunity_count = len(profits)
        profitable_count = (profits > 0).sum()
        win_rate = (profitable_count / opportunity_count * 100) if opportunity_count > 0 else 0
        mean_profit = profits.mean()
        
        # Определяем, является ли путь новым, для окрашивания
        text_color = 'black'
        if new_tickers:
            path_assets = set(path_name.split('->'))
            all_new_assets = {asset for ticker in new_tickers for asset in ticker.split('/')}
            if not path_assets.isdisjoint(all_new_assets):
                text_color = 'red'

        stats_text = (
            f'--- Path: {path_name} ---\n'
            f'Opportunities: {opportunity_count}\n'
            f'Profitable (>0%): {profitable_count} ({win_rate:.2f}%)\n'
            f'Mean Profit: {mean_profit:.4f}%'
        )

        inner_grid = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=outer_grid[i], hspace=0.6, height_ratios=[0.5, 2, 1])

        ax_stats = plt.Subplot(fig, inner_grid[0])
        ax_stats.text(0.5, 0.5, stats_text, ha='center', va='center', fontsize=12, color=text_color, 
                      bbox=dict(boxstyle='round,pad=0.5', fc='aliceblue', ec='black', lw=1))
        ax_stats.set_axis_off()
        fig.add_subplot(ax_stats)

        ax_ts = plt.Subplot(fig, inner_grid[1])
        colors = ['green' if p >= 0.3 else 'yellow' if p > 0 else 'red' for p in profits]

        max_points_for_plot = 2000
        if opportunity_count > max_points_for_plot:
            step = opportunity_count // max_points_for_plot
            path_df_plot = path_df.iloc[::step]
            colors_plot = colors[::step]
        else:
            path_df_plot = path_df
            colors_plot = colors

        ax_ts.bar(path_df_plot.index, path_df_plot['profit_percentage'], color=colors_plot, width=0.0001)
        ax_ts.axhline(0, color='black', linestyle='-', linewidth=1)
        ax_ts.axhline(min_profit_threshold, color='green', linestyle='--', linewidth=1.5)
        ax_ts.set_title('Profitability Over Time', fontsize=12, pad=10)
        ax_ts.set_ylabel('Net Profit (%)')
        ax_ts.grid(True, linestyle='--', alpha=0.5)
        
        legend_elements = [Patch(facecolor='green', label='Прибыль > 0.3%'),
                           Patch(facecolor='yellow', label='0 < Прибыль < 0.3%'),
                           Patch(facecolor='red', label='Убыток')]
        ax_ts.legend(handles=legend_elements, loc='upper left')
        fig.add_subplot(ax_ts)

        ax_hist = plt.Subplot(fig, inner_grid[2])
        ax_hist.hist(profits, bins=50, color='royalblue', alpha=0.75)
        ax_hist.axvline(0, color='red', linestyle='--', linewidth=1, label='Breakeven')
        ax_hist.axvline(mean_profit, color='green', linestyle='-', linewidth=1.5, label=f'Mean: {mean_profit:.4f}%')
        ax_hist.set_title('Distribution of Arbitrage Opportunities')
        ax_hist.set_xlabel('Gross Profit (%)')
        ax_hist.set_ylabel('Frequency')
        ax_hist.legend()
        ax_hist.grid(True, linestyle='--', alpha=0.5)
        fig.add_subplot(ax_hist)

    plt.savefig(filename, dpi=150, bbox_inches='tight')
    plt.close()
    logging.info(f"Detailed report saved to: {filename}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate a detailed arbitrage report from a session CSV file.')
    parser.add_argument('log_file', type=str, help='Path to the session CSV file.')
    parser.add_argument('output_file', type=str, help='Path to save the output PNG report.')
    parser.add_argument('exchange_name', type=str, choices=['huobi', 'binance'], help='The name of the exchange.')
    
    args = parser.parse_args()

    try:
        df = pd.read_csv(args.log_file)
        df['timestamp'] = pd.to_datetime(df['timestamp'])
        df.set_index('timestamp', inplace=True)
        
        print(f"Generating report from {args.log_file}...")
        generate_report(df, args.output_file, args.exchange_name)
        print(f"Report saved to {args.output_file}")

    except FileNotFoundError:
        print(f"Error: The file {args.log_file} was not found.")
    except Exception as e:
        print(f"An error occurred: {e}")
